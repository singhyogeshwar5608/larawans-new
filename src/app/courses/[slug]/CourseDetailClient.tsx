...
{/* ==================== PRICING CTA BAR ==================== */}
      <div className="bg-gray-50/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 sm:py-7">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-3 sm:gap-6 bg-white rounded-2xl border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.1)] relative overflow-hidden">
            ...
            {/* LEFT — Pricing */}
            <div className="flex items-center gap-3 sm:gap-6 px-4 sm:px-7 py-3 sm:py-6 lg:py-7 w-full lg:w-auto">
              {/* Discount Badge */}
              <div className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 text-[10px] sm:text-xs font-bold uppercase tracking-wide px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-emerald-100">
                <Eye className="w-3.5 h-3.5" /> {discount}% OFF
              </div>
              <div>
                <div className="flex items-baseline gap-3">
                  <span className="text-[1.5rem] sm:text-[2rem] lg:text-[2.25rem] font-extrabold text-gray-900 leading-none">{course.price}</span>
                  <span className="text-sm sm:text-lg text-slate-400 line-through font-medium">{originalPrice}</span>
                </div>
                <p className="text-[11px] sm:text-[13px] text-red-500 font-semibold mt-0.5 sm:mt-1.5 flex items-center gap-1 sm:gap-1.5">
                  <Flame className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Limited Time Offer!
                </p>
              </div>
            </div>
            ...
            {/* CENTER — Countdown */}
            <div className="flex flex-col items-center gap-1.5 sm:gap-3 px-3 sm:px-6 py-2 sm:py-4">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="h-px w-5 sm:w-8 bg-slate-200" />
                <span className="text-[11px] sm:text-[13px] font-semibold text-slate-500 uppercase tracking-wider">Offer ends in</span>
                <div className="h-px w-5 sm:w-8 bg-slate-200" />
              </div>
              <div className="flex gap-1.5 sm:gap-2.5">
                {["Days", "Hours", "Mins", "Secs"].map((label, i) => {
                  ...
                  return (
                    <div key={label} className="text-center bg-indigo-50/70 rounded-lg sm:rounded-xl px-2 sm:px-3.5 py-1.5 sm:py-2.5 min-w-[44px] sm:min-w-[58px] border border-indigo-100/60">
                      <div className="text-base sm:text-xl font-extrabold text-indigo-600 font-mono leading-none">{pad(val)}</div>
                      <div className="text-[8px] sm:text-[10px] text-slate-400 mt-0.5 sm:mt-1 uppercase tracking-wider font-medium">{label}</div>
                    </div>
                  );
                })}
              </div>
            ...
            {/* RIGHT — CTAs */}
            <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 px-4 sm:px-7 py-3 sm:py-6 lg:py-7 w-full lg:w-auto">
              <button className="w-full sm:w-auto inline-flex items-center justify-center gap-2 text-white font-bold text-[0.85rem] sm:text-[0.95rem] py-2.5 sm:py-3.5 px-6 sm:px-8 rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-emerald-200/60 active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #10B981, #059669)", boxShadow: "0 4px 14px rgba(16,185,129,0.3)" }}>
                <Zap className="w-5 h-5" /> Enroll Now
              </button>
              <button className="w-full sm:w-auto inline-flex items-center justify-center gap-2 font-bold text-[0.85rem] sm:text-[0.95rem] py-2.5 sm:py-3.5 px-6 sm:px-8 rounded-xl transition-all duration-200 hover:shadow-md active:scale-[0.98] text-slate-600 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50">
                  ...
                  {course.techStack.filter((t: any) => t.name !== "Midjourney").map((tech: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="inline-flex items-center gap-1.5 sm:gap-2.5 rounded-full pl-1 sm:pl-1.5 pr-2.5 sm:pr-4 py-1 sm:py-1.5 transition-all duration-200 border hover:shadow-[0_2px_8px_rgba(0,0,0,0.1)]" style={{ background: tech.color + "10", borderColor: tech.color + "30" }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = tech.color + "20" }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = tech.color + "10" }}>
                        ...
                        <span className="text-[0.65rem] sm:text-[0.875rem] font-semibold" style={{ color: tech.color }}>{tech.name}</span>
                      </div>
                    );
                  })}
                </div>
