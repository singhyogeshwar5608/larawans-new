...
/** Maps Tailwind color names to hex for inline gradient conversion */
const TW_COLOR: Record<string, string> = {
  'indigo-600': '#4f46e5', 'violet-600': '#7c3aed', 'violet-800': '#6d28d9',
  'purple-600': '#9333ea', 'purple-800': '#6b21a8', 'pink-700': '#be185d',
  'pink-800': '#9d174d', 'blue-600': '#2563eb', 'blue-800': '#1e40af',
  'indigo-800': '#3730a3', 'indigo-900': '#312e81', 'emerald-600': '#059669',
  'teal-800': '#115e59', 'teal-600': '#0d9488', 'cyan-600': '#0891b2',
  'slate-700': '#334155', 'fuchsia-600': '#c026d3', 'rose-600': '#e11d48',
};

function twGradientCSS(classes: string, direction = 'to bottom right'): string {
  let from = '#4f46e5', to = '#6d28d9';
  for (const p of classes.trim().split(/\s+/)) {
    if (p.startsWith('from-')) from = TW_COLOR[p.slice(5)] || from;
    if (p.startsWith('to-')) to = TW_COLOR[p.slice(3)] || to;
  }
  ...
  return (
    <div className="min-h-screen bg-[#060814] text-white font-sans selection:bg-[#7c5cff] selection:text-white overflow-x-hidden antialiased">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[900px] h-[400px] blur-[120px] rounded-full" style={{ background: 'linear-gradient(to bottom, rgba(124,92,255,0.2), rgba(0,224,198,0.1), transparent)' }} />
      ...
      <main className="relative z-10 pt-24 sm:pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8 sm:space-y-10 lg:space-y-12">
        ...
        <section className="relative rounded-3xl border border-white/10 p-6 sm:p-8 lg:p-10 shadow-[0_0_60px_rgba(0,0,0,0.6)] backdrop-blur-xl overflow-hidden" style={{ background: 'linear-gradient(to bottom right, rgba(12,15,40,0.95), rgba(9,11,30,0.9), rgba(6,8,20,0.95))' }}>
          {/* Animated Neon Top Glow Accent */}
          <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: 'linear-gradient(to right, transparent, #7c5cff, #00e0c6, transparent)' }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
            ...
            <div className="lg:col-span-7 space-y-5">
              <motion.div 
                ...
                transition={{ duration: 0.4 }}
                className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-[#7c5cff]/30 text-white text-[11px] font-extrabold uppercase tracking-widest shadow-[0_0_20px_rgba(124,92,255,0.25)]" style={{ background: 'linear-gradient(to right, rgba(124,92,255,0.2), rgba(0,224,198,0.2))' }}
              >
              ...
              >
                <a
                  href="/#contact"
                  className="group relative inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-white font-extrabold text-xs shadow-[0_0_30px_rgba(124,92,255,0.35)] hover:shadow-[0_0_45px_rgba(0,224,198,0.5)] transition-all cursor-pointer overflow-hidden" style={{ background: 'linear-gradient(to right, #7c5cff, #00e0c6)' }}
                >
            ...
            >
              <div className="relative rounded-2xl border border-white/15 p-6 space-y-5 shadow-2xl overflow-hidden group" style={{ background: 'linear-gradient(to bottom right, #121538, #0d0f2b, #080a1d)' }}>
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl text-white shadow-md shadow-[#7c5cff]/30" style={{ background: 'linear-gradient(to top right, #7c5cff, #00e0c6)' }}>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
                  isActive
                    ? "text-white shadow-md shadow-[#7c5cff]/30"
                    : "text-slate-400 hover:text-white hover:bg-white/[0.05]"
                }`}
                style={isActive ? { background: 'linear-gradient(to right, #7c5cff, #00e0c6)' } : undefined}
              >
                <IconComp size={14} />
                <span>{tab.label}</span>
              </button>
        ...
        <section id="section-overview" className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-6 rounded-2xl border border-white/10 p-5 sm:p-7 shadow-xl space-y-4 flex flex-col justify-between" style={{ background: 'linear-gradient(to bottom, rgba(14,17,46,0.9), rgba(8,10,29,0.9))' }}>
          ...
          <div className="lg:col-span-6 rounded-2xl border border-white/10 p-5 sm:p-7 shadow-xl space-y-4" style={{ background: 'linear-gradient(to bottom, rgba(14,17,46,0.9), rgba(8,10,29,0.9))' }}>
            <div className="flex items-center gap-2">
        ...
        <section id="section-process" className="rounded-2xl border border-white/10 p-5 sm:p-7 shadow-xl space-y-6" style={{ background: 'linear-gradient(to bottom, rgba(14,17,46,0.9), rgba(8,10,29,0.9))' }}>
            ...
            {service.process.map((stepItem, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg text-white font-black text-[11px] shadow-sm" style={{ background: 'linear-gradient(to top right, #7c5cff, #00e0c6)' }}>
                    {stepItem.step}
                  </span>
        ...
        <section id="section-cases" className="rounded-2xl border border-white/10 p-5 sm:p-7 shadow-xl space-y-5" style={{ background: 'linear-gradient(to bottom, rgba(14,17,46,0.9), rgba(8,10,29,0.9))' }}>
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div>
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {service.projects.map((proj, idx) => (
              ...
              >
                <div className="h-24 w-full p-3 flex flex-col justify-end text-white relative" style={{ background: twGradientCSS(proj.gradient) }}>
        ...
        <section id="section-faq" className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* FAQ Accordion */}
          <div className="lg:col-span-7 rounded-2xl border border-white/10 p-5 sm:p-7 shadow-xl space-y-4" style={{ background: 'linear-gradient(to bottom, rgba(14,17,46,0.9), rgba(8,10,29,0.9))' }}>
            <div>
              <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">
          ...
          <div className="lg:col-span-5 rounded-2xl border border-white/15 p-5 sm:p-7 text-white space-y-5 flex flex-col justify-between shadow-2xl relative overflow-hidden group" style={{ background: 'linear-gradient(to bottom right, #121538, #0d0f2b, #080a1d)' }}>
            <div className="space-y-3 relative z-10">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl text-white shadow-md" style={{ background: 'linear-gradient(to top right, #7c5cff, #00e0c6)' }}>
                <Calendar size={20} />
              </div>
              ...
              <div className="space-y-2 pt-2">
                <a
                  href="/#contact"
                  className="w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl text-white text-xs font-black shadow-lg shadow-[#7c5cff]/30 transition-all cursor-pointer" style={{ background: 'linear-gradient(to right, #7c5cff, #00e0c6)' }}
                >
